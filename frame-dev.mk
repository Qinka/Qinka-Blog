blog.post: blog.html 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "tag=page" ' \
		 $(SITE_URL)/blog  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
blog.script: script/blog.js 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=script" ' \
		 $(SITE_URL)/script/blog  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
bottom.frame: frame-bottom.html 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=frame" ' \
		 $(SITE_URL)/~@123bottom  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
code.style.sheet: style-sheet/code.glob.css 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=style-sheet" ' \
		 $(SITE_URL)/highlight/default  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
fork.me.img: fork.me.png 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=frame" ' \
		 $(SITE_URL)/img/fork.me.png  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
frame.js.text: script/frame.js 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=script" ' \
		 $(SITE_URL)/script/frame.js  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
glob.icon.binary: glob.ico 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=icon" ' \
		 $(SITE_URL)/favicon.ico  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
hack.theme: style-sheet/hack.css 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=theme" ' \
		 $(SITE_URL)/theme/hack  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
index.post: index.md 
	...
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		 $(SITE_URL)/  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
nav.frame: frame-nav.html 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=frame" ' \
		 $(SITE_URL)/~@123nav  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
...
top.frame: frame-top.html 
	@$(ECHO) $(CURL_PATH) $(CURL_DETAIL)  -X PUT  \
		...
		' -F "sha-file-name=/`$(MD5) $(PRIVATE_KEY).pub`" ' \
		' -F "tag=frame" ' \
		 $(SITE_URL)/~@123top  |  $(IH_PATH) -m  -f '$(IH_DELAY)' -p '$(PRIVATE_KEY)' -d '$(SITE_DELTA)' -v | $(SHELL)
